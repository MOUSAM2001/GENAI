...
def create_advanced_prompt(style):
    ...
    **Your Main Goal:** Write a story in simple, clear and in easy English.
    **Your Task:** Create one single story that connects all the provided images in order.
    **Style Requirement:** The story should fit the '{style}' genre.
    **Core Instructions:**
    ...
    **Output Format:**
    ...
    -   **Length:** The story must be between 1 and 5 paragraphs.
    """

    # --- Add Style-Specific Instructions ---
    style_instruction = ""
    if style == "Technology":
        style_instruction = "\n**Special Section:** After the story, you MUST add a section starting with the exact tag `[TECH_INSIGHT]:` that explains a key technological concept featured in the story."
    elif style == "Murder":
        style_instruction = "\n**Special Section:** After the story, you MUST add a section starting with the exact tag `[CLUE]:` that highlights the most important clue that led to solving the murder."
    elif style == "Mystery":
...
#         return f"An unexpected error  occured during the API call: {e}"